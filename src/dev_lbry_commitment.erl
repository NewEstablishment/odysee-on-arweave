...
%%% own content addressing, rather than through a node signature. Every
%%% commitment carries `commitment-device' `lbry@1.0' and an `evidence' field
%%% naming its verification recipe (`claim', `channel', `stream',
%%% `descriptor', `blob', `transaction', or `attestation'); evidence messages
%%% themselves carry no `device' key. Messages carrying these commitments
%%% verify through `hb_message:verify' via the `commitment-device' dispatch
%%% in `dev_message'; every recipe re-derives its facts from the committed
%%% raw bytes, so a forged `evidence' value can only select a recipe that
%%% fails, never one that accepts.
-module(dev_lbry_commitment).
...
commitment(Evidence, Type, {NativeIDType, NativeIDBytes}, Committed, Extra) ->
    {
...
with_commitment(Msg, Evidence, Type, NativeIDSpec, Committed, Extra) ->
    {ID, Commitment} = commitment(Evidence, Type, NativeIDSpec, Committed, Extra),
...
descriptor_message(Raw, SDHash) ->
    case dev_lbry_stream_descriptor:parse(Raw, SDHash) of
        {ok, Descriptor} ->
...
claim_committed_list(undefined) ->
    [
        ...
        <<"claim-proof-strength">>, <<"nout">>, <<"raw-transaction">>,
        <<"txid">>
    ];
...
channel_output_message(Raw, Nout, Ancestry) ->
    maybe
        ...
        {ok, RawPublicKey} ?=
            dev_lbry_claim_proto:channel_public_key(
                maps:get(<<"message">>, Envelope)
            ),
        {ok, PublicKey} ?= dev_lbry_attestation:normalize_public_key(RawPublicKey),
...
%% the stream `sd_hash' binding -- each verified by its own recipe.
...
stream_claim_message(Raw, Nout, Ancestry) ->
    maybe
        ...
        Envelope = maps:get(<<"claim-envelope">>, ClaimMsg),
        {ok, SDHash} ?=
            dev_lbry_claim_proto:stream_sd_hash(maps:get(<<"message">>, Envelope)),
        ClaimID = maps:get(<<"claim-id">>, ClaimMsg),
        ClaimOp = maps:get(<<"claim-op">>, ClaimMsg),
        Type = claim_commitment_type(ClaimMsg),
        TxIDHex = maps:get(<<"txid">>, ClaimMsg),
...
with_attestation_commitment(StreamMsg, ChannelMsg) ->
    maybe
        true ?=
            has_evidence_commitment(ChannelMsg, <<"channel">>, #{})
        ...
        {ok, true} ?=
            valid_or_error(
                dev_lbry_attestation:verify_signature(
                    Signature,
                    Digest,
        ...
        Commitment = #{
            ...
            <<"committed">> => [
                ...
                <<"claim-op">>, <<"nout">>, <<"raw-transaction">>, <<"txid">>
            ],
...
stream_output_verification(Base, Req, Opts) ->
    maybe
        <<"stream">> ?= base_evidence(Base, Opts),
...
attestation_verification(Base, Req, Opts) ->
    maybe
        ...
        {ok, true} ?=
            valid_or_error(
                dev_lbry_attestation:verify_signature(Signature, Digest, PublicKey),
                invalid_claim_signature
            ),
...
verified_channel_evidence(Base, Req, Opts) ->
    maybe
        ...
        true ?=
            has_evidence_commitment(ChannelMsg, <<"channel">>, Opts)
                orelse {error, invalid_channel_evidence},
        true ?=
            hb_message:verify(
                ChannelMsg,
        ...
        <<"channel">> ?= base_evidence(ChannelMsg, Opts),
...
%% @doc Whether any `lbry@1.0' commitment on a message names the given
%% evidence kind.
has_evidence_commitment(Msg, Evidence, Opts) ->
    lists:member(Evidence, evidence_kinds(Msg, Opts)).

%% The evidence kinds named by a message's own `lbry@1.0' commitments.
evidence_kinds(Msg, Opts) ->
    Commitments =
    ...
    [
        maps:get(<<"evidence">>, Commitment, undefined)
     ||
        Commitment <- maps:values(Commitments),
        lbry_commitment(Commitment)
    ].

%% @doc The claim-family evidence kind of a message, derived from its own
%% commitments: the single `lbry@1.0' device serves every evidence kind, so
%% the message kind that selects the committed-key allowlist is the most
%% specific claim-family evidence the message's commitments name. Channel
%% and stream evidence extend the claim evidence key set, so they win over
%% a co-resident claim commitment. The kind is attacker-supplied either
%% way; every recipe re-derives its facts, so it only selects which
%% allowlist and pins apply.
base_evidence(Base, Opts) ->
    Kinds = evidence_kinds(Base, Opts),
    case {
        lists:member(<<"stream">>, Kinds),
        lists:member(<<"channel">>, Kinds),
        lists:member(<<"claim">>, Kinds)
    } of
        {true, _, _} -> <<"stream">>;
        {_, true, _} -> <<"channel">>;
        {_, _, true} -> <<"claim">>;
...
%% be vouched for by a co-resident commitment of the deriving evidence kind.
%% The trust boundaries verify every LBRY commitment on a message, so
%% requiring the deriving commitment's presence makes its verification
%% mandatory.
claim_committed_keys_allowed(Base, Req, Opts) ->
    maybe
        Keys = committed_keys(Req, Opts),
        ok ?= committed_subset(Req, allowed_evidence_keys(base_evidence(Base, Opts)), Opts),
        true ?=
            (not lists:member(<<"sd-hash">>, Keys))
                orelse has_evidence_commitment(Base, <<"stream">>, Opts)
                orelse {error, missing_stream_commitment},
        true ?=
            (not lists:member(<<"channel-evidence">>, Keys))
                orelse has_evidence_commitment(Base, <<"attestation">>, Opts)
...
%% @doc `value' is deterministically decoded from the committed claim
%% envelope; when it is in the committed set, it must re-derive from the
%% envelope exactly, or a forged `value' would ride on otherwise-valid
%% claim evidence.
verify_committed_value(Base, Req, Envelope, Opts) ->
    case lists:member(<<"value">>, committed_keys(Req, Opts)) of
        false ->
            ok;
        true ->
            {Fields, _Keys} = decoded_value_fields(Envelope),
            Expected = maps:get(<<"value">>, Fields, undefined),
            Actual =
                hb_cache:ensure_all_loaded(
                    hb_maps:get(<<"value">>, Base, undefined, Opts),
                    Opts
                ),
            % Compare by unsigned message id: the base arrives here in
            % different encodings (structured or TABM) depending on the
            % verification entrypoint, and the id is canonical across them.
            case value_content_id(Actual, Opts) =:= value_content_id(Expected, Opts) of
                true -> ok;
                false -> {error, committed_value_mismatch}
            end
    end.

value_content_id(Value, Opts) when is_map(Value) ->
    catch hb_message:id(Value, uncommitted, Opts);
value_content_id(Value, _Opts) ->
    Value.
        ...
        ok ?=
            vouch_channel_evidence(
                lists:member(<<"channel-evidence">>, Keys)
                    andalso
                        hb_maps:get(<<"evidence">>, Req, undefined, Opts)
                            =/= <<"attestation">>,
                Base,
                Opts
            ),
    ...
    Attestations =
        [
         ...
         ||
            Commitment <- maps:values(Commitments),
            lbry_commitment(Commitment),
            maps:get(<<"evidence">>, Commitment, undefined) == <<"attestation">>
        ],
    Valid =
        Attestations =/= []
            andalso
...
%% evidence messages, so the base is pinned to that family through its own
%% commitments; the channel and stream verifications additionally pin their
%% exact kind.
verify_claim_output(Base, Req, OutpointBytes, Opts) ->
    maybe
        true ?=
            ...
            ) orelse {error, claim_evidence_mismatch},
        ok ?= claim_committed_keys_allowed(Base, Req, Opts),
...
channel_output_verification(Base, Req, Opts) ->
    maybe
        <<"channel">> ?= base_evidence(Base, Opts),
...
%% carry an `lbry@1.0' commitment of the expected evidence kind whose native
%% identifier matches the requested key, and every LBRY commitment on the
%% message must verify. On success the message is returned with only its
%% LBRY commitments: the remote node's own transport signatures are
...
%% a specific kind can narrow the acceptable evidence kinds with the
%% `verify-remote-evidence' option.
verify_remote_read(Key, Msg, Opts) ->
    case expected_remote_commitment(Key) of
        untyped ->
            {ok, Msg};
        {ok, Kinds, NativeIDHex} ->
            require_native_commitments(
                narrow_evidence(Kinds, Opts),
                NativeIDHex,
                Key,
                Msg,
                Opts
...
narrow_evidence(Kinds, Opts) ->
    case hb_maps:get(<<"verify-remote-evidence">>, Opts, undefined, Opts) of
        Allowed when is_list(Allowed) ->
            [Kind || Kind <- Kinds, lists:member(Kind, Allowed)];
        _ ->
            Kinds
    end.

%% @doc Classify a store key as an LBRY native identifier. Returns the
%% acceptable commitment evidence kinds and the expected commitment
%% `native-id' for the key, or `untyped' when the key is not LBRY-shaped.
...
require_native_commitments(Kinds, NativeIDHex, Key, Msg, Opts) when is_map(Msg) ->
    RemoteCommitments =
        hb_cache:ensure_all_loaded(
            hb_maps:get(<<"commitments">>, Msg, #{}, Opts),
            Opts
...
require_native_commitments(_Kinds, _NativeIDHex, Key, _Msg, _Opts) ->
...
has_evidence_commitment(Commitments, Evidence) ->
    lists:any(
        fun(Commitment) ->
            lbry_commitment(Commitment)
                andalso maps:get(<<"evidence">>, Commitment, undefined) =:= Evidence
        end,
...
%% The canonical lists derive from the commitment evidence kinds alone,
%% mirroring the constructors (including their `share_committed_keys'
%% union), and every native fact is re-verified before the message is
%% accepted, so the lists are rebuilt here rather than trusted from the
%% wire.
...
evidence_committed_list(<<"blob">>, _Ancestry) ->
    [<<"blob-hash">>, <<"content-digest">>, <<"data">>];
evidence_committed_list(<<"transaction">>, _Ancestry) ->
    [<<"raw">>, <<"txid">>];
evidence_committed_list(<<"descriptor">>, _Ancestry) ->
    [<<"raw">>, <<"sd-hash">>];
evidence_committed_list(<<"claim">>, Ancestry) ->
    claim_committed_list(Ancestry);
evidence_committed_list(<<"channel">>, Ancestry) ->
    lists:sort(
        claim_committed_list(Ancestry) ++ [<<"channel-id">>, <<"public-key">>]
    );
evidence_committed_list(<<"stream">>, Ancestry) ->
    lists:sort(claim_committed_list(Ancestry) ++ [<<"sd-hash">>]);
evidence_committed_list(<<"attestation">>, _Ancestry) ->
    [
        <<"channel-evidence">>, <<"claim">>, <<"claim-id">>, <<"claim-op">>,
        <<"nout">>, <<"raw-transaction">>, <<"txid">>
    ];
evidence_committed_list(_Evidence, _Ancestry) ->
    [].
...
ancestry_with_input_parents_verifies_through_dispatch_test() ->
    ...
    {CreateRaw, _Updates, ClaimID} = dev_lbry_ancestry:test_chain(1),
    OtherCreate = dev_lbry_ancestry:test_create_tx(<<"other">>),
    Child =
    ...
    Raws = #{
        dev_lbry_tx:txid(CreateRaw) => CreateRaw,
        dev_lbry_tx:txid(OtherCreate) => OtherCreate
    },
    Fetch =
        fun(TxID) ->
...
claim_output_message_rejects_invalid_ancestry_test() ->
    ...
    BogusEntry = #{
        <<"txid">> => dev_lbry_tx:txid(CreateRaw),
        <<"nout">> => 0,
        <<"claim-op">> => <<"create">>,
        <<"claim-id">> => ClaimID,
...
evidence_tampering_fails_verification_test() ->
    ...
    Tampered = [
        relabel_evidence(blob_message(Hash, Bytes), <<"claim">>),
        relabel_evidence(TxMsg, <<"blob">>),
        relabel_evidence(ClaimMsg, <<"blob">>),
        relabel_evidence(StreamMsg, <<"claim">>),
        relabel_evidence(ChannelMsg, <<"claim">>),
        relabel_evidence(DescriptorMsg, <<"blob">>)
    ],
    lists:foreach(
        fun(Msg) ->
            ?assertEqual(
    ...
    [{ClaimID, _}] =
        [
         ...
         ||
            ...
            maps:get(<<"evidence">>, Commitment) == <<"claim">>
        ],
    Dropped = Msg#{
...
attestation_commitment_rejects_tampered_channel_params_test() ->
    ...
    % The attestation recipe pins its base to stream evidence: relabel the
    % sibling stream commitment and verify only the attestation commitment,
    % so the failure cannot come from the claim or stream commitments.
    [{StreamID, StreamCommitment}] =
        [
            {ID, Commitment}
         ||
            {ID, Commitment} <- maps:to_list(Commitments),
        ...
        hb_message:verify(
            Committed#{
                <<"commitments">> =>
                    Commitments#{
                        StreamID =>
                            StreamCommitment#{ <<"evidence">> => <<"claim">> }
                    }
            },
