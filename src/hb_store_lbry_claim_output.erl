...
%%% output is parsed into a claim-output message carrying a native
%%% `lbry@1.0' commitment with `claim' evidence. With the `kind' store
%%% option set to `channel', the output must be a channel claim and the
%%% returned message carries the normalized channel public key under
%%% `channel' evidence instead. With `kind' set to `stream', the output must
%%% be a stream claim and the returned message additionally carries the
%%% descriptor `sd_hash' under a `stream'-evidence commitment.
%%%
...
read_walks_ancestry_for_updates_test() ->
    ...
    try
        ...
        UpdateTxID = dev_lbry_tx:txid(UpdateRaw),
        {ok, Msg} =
            read(
                Store,
...
read_degrades_to_asserted_without_ancestry_test() ->
    ...
    application:ensure_all_started(inets),
    {_CreateRaw, [UpdateRaw], _ClaimID} = dev_lbry_ancestry:test_chain(1),
...
chain_proxy_server(Raws) ->
    Index =
        maps:from_list(
            [{dev_lbry_tx:txid(Raw), hb_util:to_hex(Raw)} || Raw <- Raws]
        ),
    Handler =
        fun(Req) ->
