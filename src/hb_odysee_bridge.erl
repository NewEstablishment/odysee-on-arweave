%%% @doc High-level bridge between the Odysee SDK proxy and the LBRY
%%% evidence stores. The SDK is used purely as a locator: it points at
%%% claim outpoints and sd-hashes, but every fact the bridge returns is
%%% re-derived from raw on-chain transactions, claim protobufs, and
%%% content-addressed blobs fetched through the stores, with every native
%%% `lbry@1.0' commitment verified through `hb_message:verify'. Also
%%% provides stream reassembly and ranged reads over descriptor-addressed
%%% encrypted blob streams.
...
%% including its native `lbry@1.0' blob commitment.
...
%% transaction message, including its native `lbry@1.0' transaction
%% commitment.
...
%% returning a message with a native `lbry@1.0' claim commitment.
...
%% store, returning a message with a native `lbry@1.0' channel commitment
...
%% store, returning a message with native `lbry@1.0' claim and stream
%% commitments.
...
%% descriptor message, including its native `lbry@1.0' descriptor
%% commitment.
descriptor_message(SDHash, Opts) ->
    case read_blob(SDHash, Opts) of
        {ok, Raw} -> dev_lbry_commitment:descriptor_message(Raw, SDHash);
        Error -> Error
    end.
...
stream_graph(ClaimIDOrName, Opts) ->
    maybe
        {ok, Claim} ?= hb_odysee_client:claim(ClaimIDOrName, Opts),
...
verified_stream(ClaimIDOrName, Opts) ->
    maybe
        ...
        {ok, ParsedTx} ?=
            dev_lbry_tx:parse(maps:get(<<"raw-transaction">>, StreamMsg0)),
...
stream_size(SDHash, Opts) ->
    maybe
        ...
        {ok, Size} ?= dev_lbry_stream_descriptor:stream_size(Descriptor, Fetch),
        {ok, #{
            <<"sd-hash">> => hb_util:to_lower(SDHash),
            <<"byte-size">> => Size
        }}
    ...
    case maps:get(<<"signed">>, Envelope, false) of
        ...
        true ->
            maybe
                ...
                {ok, ChannelClaim} ?=
                    hb_odysee_client:claim_search(EnvelopeChannelID, Opts),
