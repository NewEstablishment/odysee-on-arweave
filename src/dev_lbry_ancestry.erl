...
verify_walk(ChildRaw, ChildNout, Entries, DepthLimit) ->
    maybe
        {ok, Tx} ?= dev_lbry_tx:parse(ChildRaw),
...
entry_evidence(Entry) when is_map(Entry) ->
    maybe
        ...
        {ok, ParentTx} ?= dev_lbry_tx:parse(Raw),
...
verify_spend(ChildTx, InputIndex, ParentOutput) ->
    maybe
        ...
        true ?=
            dev_lbry_tx:hash160(PublicKey) == PubKeyHash
                orelse {error, spend_public_key_mismatch},
        {ok, Uncompressed} ?=
            dev_lbry_attestation:public_key_to_uncompressed(PublicKey),
        Digest =
            dev_lbry_tx:signature_hash(
                ChildTx,
                InputIndex,
...
test_key() ->
    ...
    {PrivateKey, Compressed, dev_lbry_tx:hash160(Compressed)}.
...
test_update_tx(Name, ClaimID, Spends, SignOpts) ->
    ...
    UnsignedInputs =
        [
            serialized_input(
                dev_lbry_tx:double_sha256(ParentRaw),
                ParentNout,
                <<>>
            )
         ||
    ...
    {ok, UnsignedTx} = dev_lbry_tx:parse(Unsigned),
    [{ClaimParentRaw, ClaimParentNout} | RestSpends] = Spends,
    {ok, ParentTx} = dev_lbry_tx:parse(ClaimParentRaw),
    ParentOutput =
    ...
    SignedInputs = [
        serialized_input(
            dev_lbry_tx:double_sha256(ClaimParentRaw),
            ClaimParentNout,
            ScriptSig
        )
    |
        [
            serialized_input(dev_lbry_tx:double_sha256(ParentRaw), ParentNout, <<>>)
         ||
...
test_chain(Updates) ->
    ...
    {ok, CreateTx} = dev_lbry_tx:parse(CreateRaw),
...
fixture_fetch(Raws) ->
    Index =
        maps:from_list(
            [{dev_lbry_tx:txid(Raw), Raw} || Raw <- Raws]
        ),
    fun(TxID) ->
...
real_claim_spend_signature_verifies_test() ->
    ...
    {ok, Tx} = dev_lbry_tx:parse(Update),
    {ok, ParentTx} = dev_lbry_tx:parse(Parent),
    ?assertEqual(
...
verify_walk_fails_on_unrelated_parent_test() ->
    ...
    {ok, Tx} = dev_lbry_tx:parse(UpdateRaw),
