%%% @doc Tier-2 channel attestation for signed LBRY claims: verifies the
%%% on-chain ECDSA (secp256k1) signature embedded in a claim envelope against
%%% the signing channel's public key, and checks that the envelope's embedded
%%% signing-channel hash binds to the channel's claim id.
%%%
%%% The signature digest is `sha256(first_input_piece || channel_hash ||
%%% claim_protobuf)', per LBRY consensus. Channel public keys are accepted in
%%% compressed, uncompressed, and legacy DER/SPKI encodings and normalized to
%%% the 33-byte compressed form; off-curve points and non-canonical field
%%% elements fail closed.
-module(dev_lbry_attestation).
-export([
    verify/3,
    signature_digest/2,
    verify_signature/3,
