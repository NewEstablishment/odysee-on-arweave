%%% @doc Parser for raw LBRY transactions: the legacy Bitcoin wire format
%%% extended with `OP_CLAIM_NAME'/`OP_UPDATE_CLAIM' claim scripts. Decodes
%%% transactions into maps, derives txids and claim ids, and computes the
%%% legacy `SIGHASH_ALL' digest used by the ancestry verifier.
-module(dev_lbry_tx).
-export([
    parse/1,
    parse_hex/1,
