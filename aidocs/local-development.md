...
When `ODYSEE_HYPERBEAM_NODE_API` is set, the branch defaults the browser to HyperBEAM mode and routes canonical reads through HyperBEAM devices. The browser mode key is `odysee-hyperbeam-mode`; valid values are `original` and `hyperbeam`. Legacy stored `hybrid` values are normalized to `hyperbeam`.
