...
- [Feature Flows](#feature-flows)
  ...
  - [Live Streaming and Replays](#live-streaming-and-replays)
  - [Search](#search)
...
| `hyperstream@1.0` | `transport-key`, `create`, `join`, `heartbeat`, `leave`, `signal`, `events`, `session`, `update`, `record`, `close` | Forge-packaged generic node-local coordination for live media sessions. It manages sealed signaling, peer leases, cursor-based event delivery, source metadata, and immutable recording-manifest links without interpreting WebRTC or carrying media. |
...
`hyperstream@1.0` is a reusable live-session control plane under
`hyperbeam/src/preloaded/hyperstream/`. It is not an Odysee adapter. A session
contains bounded opaque metadata, an owner peer, leased participants, a monotonic
event cursor, and targeted signal envelopes. Clients exchange offers, answers,
trickled ICE candidates, renegotiation messages, or encrypted application
messages through `signal`; the device preserves their bodies and ordering but
does not parse them. `events` exposes only signals addressed to the authenticated
peer. Membership and lifecycle events are visible to all current members.
The standalone demo under `hyperbeam/demos/hyperstream/` supplies separate
broadcaster and watch pages around a hybrid media adapter. The broadcaster first
creates its signed Hyperstream session. The adapter joins that session through
its internal HyperBEAM origin, checks the claimed publisher's live owner state,
and returns a short-lived one-time challenge. The owner proves control with a
targeted Hyperstream signal bound to the exact publisher and adapter
generations. The HTTP completion call returns only acceptance; publish and
release credentials are delivered only in an adapter-to-owner targeted signal.
The adapter then accepts one browser WHIP or, where enabled, external OBS/RTMP
contribution. MediaMTX remuxes that source into one full-segment fragmented MP4
HLS rendition, and the owner publishes a descriptor without publish credentials
through `update` only after the HTTPS manifest is live. Hls.js and P2P Media
Loader viewers always retain that HTTPS path while opportunistically exchanging
encoded segments over WebRTC data channels discovered through a
deployment-owned WebTorrent tracker. The Hyperstream device remains unchanged
and carries no media or segment traffic.

Allocation is serialized, idempotent for a session/publisher pair, and capped
across active MediaMTX paths and pending reservations; the configurable maximum
cannot exceed 100. Media path names contain an HMAC-derived authenticity suffix.
WHIP/RTMP publish credentials expire after 15 minutes by default, while a
separate media-scoped release token authorizes teardown. The tracker URL has its
own expiry and HMAC capability tied to the active media reservation. That
read-side connection capability is intentionally present in owner-controlled
playback metadata and cannot publish or release media. Each tracker connection
may announce one swarm and is subject to per-client connection limits plus
first-announce and idle deadlines. Open tracker sockets close when their
capability expires and are revoked when the media reservation is released.

The generic local MediaMTX configuration supports plaintext RTMP testing. The
public demo2 deployment disables RTMP, runs MediaMTX as a dedicated hardened
service account, keeps its HLS, WHIP HTTP, API, and metrics listeners on
loopback, and exposes only browser-WHIP ICE on `8189/udp` and `8189/tcp` in
addition to the HTTPS/WSS edge on port 443.

This makes viewer playback independent of inbound firewall rules, UDP, tracker
availability, or P2P success: ordinary HTTPS on port 443 is the fallback. TURN
is not required for playback, although it can improve the P2P connection ratio
on restrictive networks and then consumes relay bandwidth. Connected P2P peers
can learn network-address information about one another; relay-only TURN is
needed when that privacy tradeoff is unacceptable. Browser-WHIP and RTMP ingest
have separate server port requirements. The Odysee frontend still uses its
existing livestream path; this demo is not yet integrated into the product UI.

Peers authenticate with one verified request signer or with a random capability
token generated by the client before `create`/`join`. The standalone demo signs
every request with a browser-generated RSA identity, so that signer is the peer
membership credential and no peer token is needed. Generic restricted
applications can still use random peer and join capabilities, but must deliver
them out of band and include them only inside the sealed request body. They must
never appear in a URL, metadata, diagnostics, or durable recording descriptor.
Peer IDs, roles, and metadata remain application-supplied labels; an open
session cannot prevent a participant from being first to claim an unused peer
ID.

HTTP clients first call the public `transport-key` operation; the standalone
demo signs this bootstrap and every later request. Clients then exchange `hs1`
request and response envelopes produced with P-256 ECDH,
HKDF-SHA256, and AES-256-GCM. The operation and key ID are authenticated with
each envelope. Ordinary HyperBEAM request logging, hooks, signing, and store
paths therefore see flat ciphertext instead of signaling, credentials, or
session metadata. The trusted node process still decrypts and coordinates that
data; this is transport sealing, not end-to-end encryption from the node
operator. All responses remain `no-store`, but that cache directive does not
disable server-side persistence. A stock node's default `store-all-signed`
behavior can retain encrypted request messages at heartbeat and polling
frequency. A dedicated Hyperstream node should set `store-all-signed=false` and
retain only suitable edge metrics for this ephemeral traffic. That is ordinary
node configuration, not a core patch.

Hyperstream uses the normal direct-device dispatch and Forge device-loading
contract. It does not require HTTP, hook, message, meta, or store changes in the
HyperBEAM core. An operator can bake it into a preloaded store or load its
signed Forge package through the standard trusted-device configuration.
Deployments must preserve the literal device route, require HTTPS outside
loopback, enforce ingress body and rate limits, avoid edge body logging, and
route every request for one session to the same node.

Media remains outside this signaling mailbox:

- A direct WebRTC publisher, relay, viewer, SFU, or RTMP gateway joins as a peer
  and advertises application-defined capabilities in peer/session `metadata`.
- The standalone adapter terminates one WHIP or RTMP ingest in MediaMTX and
  emits full-segment fMP4 HLS. The owner publishes its public HTTPS manifest,
  swarm ID, stream epoch, tracker URL, and segment target with `update`; the
  generic device does not interpret those fields or endpoint schemes.
- Viewers use Hls.js plus P2P Media Loader to share encoded HLS segments when
  WebRTC data channels are reachable. They use the same origin HLS segments over
  HTTPS whenever P2P is absent or fails; segment requests are never device
  signals. The player normalizes MediaMTX's changing bandwidth and resolution
  attributes in its one-rendition master playlist so identical content does not
  fragment into different peer identities.
- `record` accepts a bounded immutable segment locator after the segment bytes
  have been written through a normal HyperBEAM store path. Each call writes an
  immutable `hyperstream-recording-segment@1.0` entry linked to the previous
  head and a live `hyperstream-replay@1.0` snapshot, so a restart does not erase
  the latest replay locator. `close` writes the final closed replay manifest.
  Playback reads a returned `replay-id` and its segment chain through ordinary
  ID/store reads.

MediaMTX remuxes but does not transcode. The browser source is H.264, and OBS
should publish H.264/AAC with a two-second keyframe interval to match the
two-second HLS segment target. The current adapter leaves MediaMTX recording
disabled and does not write finalized segments to a HyperBEAM store or call
`record`, so the device primitives exist but replay is not wired into the demo.
For every P2P-delivered segment, the viewer fetches an expected SHA-256 digest
from the same HTTPS application origin, hashes the received bytes locally, and
fails closed on a mismatch or unavailable digest. Trusted HTTPS-origin segments
start the same check in the background but do not wait for it, preserving the
guaranteed fallback when the digest service is unavailable. The digest service
reads the segment through the viewer's MediaMTX HLS session and keeps bounded
positive caches plus session-scoped negative caches behind request-rate and
global-concurrency limits. This trusts the live HTTPS origin; it is not a signed
immutable HyperBEAM manifest and does not provide offline or historical
verification.

Fatal player errors use a bounded four-attempt recovery sequence with capped
exponential delays. A P2P runtime failure first switches to HTTP-only HLS, and a
stable 30-second playback interval resets the recovery budget. Exhaustion is
shown as a terminal viewer error instead of creating an unbounded retry loop.

Coordination state, the transport key, SDP, ICE, network addresses, and
capability hashes live only in bounded node memory. A restart rotates the
transport key and loses sessions, so clients must fetch the new key, rejoin, and
renegotiate. Recording entries survive because they contain locators and
adapter-supplied public metadata. `record` is fail-closed to the configured
recording-signer allowlist, falling back to `cache_writers` when that option is
not set. Segment count and aggregate descriptor-byte limits bound each replay
chain, and deterministic manifests make a failed close retry safe. The device
validates immutable locators but cannot detect secrets in other descriptor
fields; recording adapters must keep those fields public and separately manage
store capacity, retention, and garbage collection. The full request contract
and limits are documented in
[`hyperbeam/docs/build/hyperstream.md`](hyperbeam/docs/build/hyperstream.md).
...
### Standalone Hyperstream demo environment

These server-only settings join Hyperstream admission to the MediaMTX adapter:

| Variable | Default | Purpose |
| --- | --- | --- |
| `HYPERSTREAM_HYPERBEAM_INTERNAL` | Public `HYPERBEAM_ORIGIN`, then `http://127.0.0.1:18785` | Internal node origin used to join the session and verify the targeted owner proof before media allocation. |
| `HYPERSTREAM_MEDIAMTX_API` | `http://127.0.0.1:9997` | Credential-free loopback MediaMTX API origin used for active-path admission and cleanup. |
| `HYPERSTREAM_MEDIAMTX_HLS_INTERNAL` | `http://127.0.0.1:8888` | Loopback HLS origin used to calculate trusted live-segment SHA-256 references. |
| `HYPERSTREAM_MAX_MEDIA_SESSIONS` | `64` | Adapter-wide ceiling across active MediaMTX paths and pending reservations. |
| `HYPERSTREAM_MAX_ADMISSION_CHALLENGES` | `128` | Maximum simultaneous owner-proof challenges. |
| `HYPERSTREAM_ADMISSION_CHALLENGE_TTL_SECONDS` | `30` | Lifetime of a one-time targeted owner-proof challenge; accepted range is 15 through 40 seconds. |
| `HYPERSTREAM_TRACKER_TOKEN_TTL_SECONDS` | `86400` | Lifetime of a media-reservation-scoped tracker URL. |

The media publish-token lifetime defaults to 900 seconds through
`HYPERSTREAM_MEDIA_TOKEN_TTL_SECONDS`. `HYPERSTREAM_MAX_MEDIA_SESSIONS` accepts
only values from 1 through 100. A persistent media-token secret lets an adapter
restart recover HLS reads, digests, tracker reconnection, and release for an
HMAC-authenticated path that MediaMTX still reports active; it does not make
MediaMTX or HyperBEAM live state durable. See the standalone demo README for the
public origins, tracker, rate limits, and deployment service configuration.
...
| `hyperstream-namespace` | Isolates one node-local coordinator state space; default `default`. |
| `hyperstream-peer-ttl-ms`, `hyperstream-session-ttl-ms`, `hyperstream-event-ttl-ms`, `hyperstream-tombstone-ttl-ms` | Lease, signal-retention, and closed-session retention windows. |
| `hyperstream-max-sessions`, `hyperstream-max-peers-per-session`, `hyperstream-max-events-per-session`, `hyperstream-max-event-bytes-per-session` | Node/session memory ceilings for live coordination. |
| `hyperstream-max-signal-bytes`, `hyperstream-max-metadata-bytes`, `hyperstream-max-read-events` | Per-request and event-read bounds. |
| `hyperstream-max-recording-segments-per-session`, `hyperstream-max-recording-descriptor-bytes-per-session` | Durable replay-chain segment and aggregate descriptor-byte ceilings. |
| `hyperstream-max-pending-calls-per-session` | Per-coordinator mailbox admission ceiling. |
| `hyperstream-recording-signers` | Signer addresses allowed to call `record`; falls back to `cache_writers`, and an empty list disables recording. |
...
- Hyperstream demo playback always retains an HTTPS HLS path, while P2P segment
  exchange is opportunistic and exposes connected peers' network-address
  information. The default P2P configuration is STUN-only; deployments must
  supply TURN configuration when relay reachability or relay-only privacy is
  required. Peer segments are checked against a SHA-256 value supplied by the
  trusted live HTTPS origin, not a signed immutable HyperBEAM manifest, and fail
  closed if it cannot be obtained. Trusted HTTPS delivery starts the check in
  the background but is not gated by that live digest service.
  MediaMTX serves one source rendition and does not transcode, recording/replay
  is not wired to the device primitives, and the complete path has not been
  load-tested with hundreds of viewers.
