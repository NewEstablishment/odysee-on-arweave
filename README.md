...
broadcaster and watch pages. The broadcaster creates one direct WebRTC
connection per independent viewer while both pages expose sanitized signaling,
device, connection, cursor, and RTP diagnostics. It caps the publisher at eight
media connections and limits repeated negotiation per viewer generation. Direct
ICE candidates remain visible to the connected peer; relay-only TURN or an SFU
is required when peer IP privacy matters. The Odysee frontend still uses its
existing Livestream API, WHIP ingest, and OME playback paths; Odysee wiring, an
RTMP adapter, automatic segmentation, and replay UI are separate integration
work.
...
- An RTMP ingest adapter terminates RTMP and produces the actual WebRTC, HLS, or
  immutable segment output. The owner publishes its ingest/playback descriptors
  with `update`; the generic device does not interpret endpoint schemes.
