%%% @doc Parsing and verification of LBRY stream descriptors. A stream
%%% descriptor is a JSON blob naming the AES-128-CBC key, the ordered list
%%% of encrypted data blobs (each with its IV, length, and SHA-384 hash),
%%% and a zero-length terminator blob. Every field is validated
%%% fail-closed: hex fields must decode to their exact expected sizes,
%%% non-final data blobs must be exactly the maximum blob size, and the
%%% terminator must carry no hash. Descriptor and blob identifiers remain
%%% lowercase hex, as they are LBRY-native display identity.
