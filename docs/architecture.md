...
New and copied playlists use private snapshots by default. A small browser
WebCrypto client implements the WeaveMail 1.0 envelope format locally: it uses
a fresh AES-256-GCM key per snapshot and wraps that key with RSA-OAEP/SHA-256.
The RSA key pair is generated by the browser, the private key is non-exportable,
and both CryptoKeys are persisted in IndexedDB under the verified native cookie
owner. The public message reveals the encryption format, purpose, verified
owner, recipient key ID, ciphertext, wrapped key, IV, tag, and signature scope,
but not playlist/profile metadata or items. There is no private-playlist or
