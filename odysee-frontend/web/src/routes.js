...
const HYPERBEAM_AUTH_DEVICE_PATHS = new Set([
  ...
  '/~odysee-private@1.0/owner',
  '/~odysee-private@1.0/seal',
  '/~odysee-private@1.0/open',
...
function decodeThumbnail(requestBody) {
  const raw = requestBody.content_base64 || requestBody['content-base64'] || requestBody.data || '';
...
function hbSignedHeaders(method, path, contentType, bodyBuf) {
  ...
  const sig = crypto.sign('sha512', Buffer.from(base), {
    key: privateKey,
