...
async function materializeHomepageData(homepageData, dependencies = {}) {
  ...
  const warmChannelsMany =
    dependencies.warmChannelsMany ||
    (dependencies.warmMany || dependencies.warm ? warmMany : hyperbeamNodeWarmImmutableChannels);
  ...
  const configuredCategoryPoolSize = positiveInteger(dependencies.categoryPoolSize);
  ...
  const selections = await mapWithConcurrency(jobs, QUERY_CONCURRENCY, async (job) => {
    ...
    const categoryPoolSize = Math.max(pageSize, configuredCategoryPoolSize || pageSize * CATEGORY_CANDIDATE_MULTIPLIER);
    const selectionKey = `${job.locale}/${job.categoryId}`;
    const candidatePageSize = pageSize * CATEGORY_CANDIDATE_MULTIPLIER;
    const candidatePageCount =
      candidatePageCounts.get(selectionKey) || Math.max(1, Math.ceil(categoryPoolSize / candidatePageSize));
    ...
    return {
      ...
      selectedIds: mergePinnedIds(
        dynamicIds,
        pinnedIds,
        Math.max(categoryPoolSize, candidatePageSize * candidatePageCount)
      ),
      pageSize,
      categoryPoolSize,
  ...
  const bannerSelections = await mapWithConcurrency(bannerEntries, QUERY_CONCURRENCY, async ([sourceItem, uri]) => {
    const bannerClaim = bannerClaims?.[uri];
    const channelClaimId = String(bannerClaim?.claim_id || bannerClaim?.['claim-id'] || '');
    if (!channelClaimId) throw new Error(`Homepage banner ${uri} has no channel claim id`);
    const result = await searchWithRetry(
      search,
      {
        ...
        page: 1,
        page_size: BANNER_MEDIA_COUNT * CATEGORY_CANDIDATE_MULTIPLIER,
        no_totals: true,
        release_time: `<${nowSeconds}`,
        exclude_shorts: true,
      },
      searchRetries,
      searchRetryDelayMs
    ...
    const selectedIds = Array.from(new Set(items.map(immutableId).filter(Boolean)));
    return {
      sourceItem,
      uri,
      selectedIds,
      pageSize: BANNER_MEDIA_COUNT,
      expectedSize: BANNER_MEDIA_COUNT,
      signingChannelByMediaId: new Map(selectedIds.map((mediaId) => [mediaId, channelClaimId])),
      sourceLocatorById: new Map(
        items.map((item) => [immutableId(item), sourceLocator(item)]).filter(([id, locator]) => id && locator)
      ),
    };
  });
  const contentSelections = [...selections, ...bannerSelections];
  const channelClaimIds = Array.from(
    new Set(
      contentSelections.flatMap((selection) =>
  ...
  const channelIdsByClaimId = new Map(
    [...channelItems, ...Object.values(bannerClaims)]
      .map((item) => [String(item.claim_id || item['claim-id'] || ''), immutableId(item)])
      .filter(([claimId, id]) => claimId && id)
  );
  ...
  const selectedIds = Array.from(new Set(contentSelections.flatMap((selection) => selection.selectedIds)));
  ...
  const mediaLocatorById = new Map(contentSelections.flatMap((selection) => Array.from(selection.sourceLocatorById)));
  ...
  const channelEvidenceResults = await retryWarmObjects(
    channelWarmResults.filter(([, ok]) => ok).map(([id]) => id),
    warmChannelsMany,
    warmRetries,
    warmRetryDelayMs,
    new Map()
  );
  const warmedChannels = new Set(channelEvidenceResults.filter(([, ok]) => ok).map(([id]) => id));
  ...
  const incompleteSelections = contentSelections.filter(
    (selection) => selection.selectedIds.length < selection.expectedSize
  );
  if (incompleteSelections.length) {
    ...
    const incompleteCategories = incompleteSelections
      .map(
        (selection) =>
          `${selection.categoryId ? `${selection.locale}/${selection.categoryId}` : `banner/${selection.uri}`} ` +
          `(${selection.selectedIds.length}/${selection.expectedSize})`
      )
      .join(', ');
    throw new Error(
  ...
  selections.forEach(({ locale, categoryId, selectedIds: ids, poolIds, signingChannelByMediaId, categoryPoolSize }) => {
    materialized[locale].categories[categoryId].immutableIds = ids;
    materialized[locale].categories[categoryId].immutablePoolIds = poolIds.slice(0, categoryPoolSize);
  ...
  bannerEntries.forEach(([sourceItem, uri]) => {
    const selection = bannerSelections.find((entry) => entry.sourceItem === sourceItem);
    const immutableIds = selection?.selectedIds.slice(0, BANNER_MEDIA_COUNT) || [];
    const immutableSigningChannelIds = Object.fromEntries(
      immutableIds.map((mediaId) => [mediaId, bannerIdsByUri.get(uri)]).filter(([, channelId]) => channelId)
    );
    for (const homepage of Object.values(materialized)) {
      for (const item of homepage?.featured?.items || []) {
        if (item.url === sourceItem.url) {
          item.immutableId = bannerIdsByUri.get(uri);
          item.immutableIds = immutableIds;
          item.immutableSigningChannelIds = immutableSigningChannelIds;
        }
      }
    }
  });
