...
test('materialization resolves and warms featured banner targets', async () => {
  ...
  const result = await materializeHomepageData(
    ...
    {
      search: async (params) => ({
        items: params.channel_ids
          ? [0, 1, 2].map((index) => ({
              immutable_id: `banner-media:${index}`,
              channel_claim_id: 'channel-claim',
            }))
          : [{ immutable_id: 'media:0' }],
      }),
      resolve: async () => ({
        'lbry://@channel#9': { claim_id: 'channel-claim', immutable_id: 'channel:0' },
      }),
      warm: async (id) => {
        warmed.push(id);
        return true;
  ...
  assert.deepEqual(result.en.featured.items[0].immutableIds, ['banner-media:0', 'banner-media:1', 'banner-media:2']);
  assert.deepEqual(
    new Set(warmed),
    new Set(['media:0', 'channel:0', 'banner-media:0', 'banner-media:1', 'banner-media:2'])
  );
});
...
test('materialization rejects a present channel whose exact evidence is not hydrated', async () => {
  await assert.rejects(
    materializeHomepageData(
      {
        en: {
      ...
      {
        search: async (params) => ({
          items: params.claim_ids
            ? [{ claim_id: 'channel-claim', immutable_id: 'channel:0' }]
            : [{ immutable_id: 'media:0', channel_claim_id: 'channel-claim' }],
        }),
        warmMany: async (ids) => ids.map((id) => [id, true]),
        warmChannelsMany: async (ids) => ids.map((id) => [id, false]),
        warmRetries: 0,
        candidateExpansionRounds: 0,
      }
    ),
    /Failed to cache 1 homepage objects/
  );
