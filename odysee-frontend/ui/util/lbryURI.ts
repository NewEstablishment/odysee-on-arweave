...
const hyperbeamUploadIdMaxLength = 128;
...
function parseURIModifier(modSeperator: string | null | undefined, modValue: string | null | undefined) {
  ...
  if (claimId && !isValidClaimIdModifier(claimId)) {
...
function isValidClaimIdModifier(claimId: string) {
  return (
    (claimId.length <= claimIdMaxLength && /^[0-9a-f]+$/.test(claimId)) ||
    (claimId.length <= hyperbeamUploadIdMaxLength && /^[0-9A-Za-z_-]+$/.test(claimId))
  );
}
