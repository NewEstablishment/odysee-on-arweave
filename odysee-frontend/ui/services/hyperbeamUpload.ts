...
import { HYPERBEAM_DEVICE, hyperbeamDevicePostParams64, hyperbeamDeviceUrl, hyperbeamNodeBase } from 'util/hyperbeamDevices';
...
export async function deleteHyperbeamPublish(uploadId: string): Promise<void> {
  return deleteThroughHyperbeam(
    {
      claim_id: uploadId,
      hyperbeam: {
        'record-id': uploadId,
...
export async function listHyperbeamPublishes(
  ...
  if (!isHyperbeamFullMode() || !hyperbeamNodeBase()) return [];

  const channelIds = (filters.channelIds || []).filter(Boolean).map(String);
  const url = hyperbeamDeviceUrl(
    HYPERBEAM_DEVICE.upload,
    'list',
    channelIds.length ? { channel_ids: channelIds.join(',') } : {}
  );
  if (!url) return [];
...
function normalizeListedClaim(claim: any) {
  const recordId = hyperbeamClaimRecordId(claim) || claim.claim_id || claim['claim-id'];
  const mediaUrl = absoluteHyperbeamUrl(claim.streaming_url || claim.download_url || claim.value?.source?.url);

  return {
    ...claim,
    claim_id: claim.claim_id || recordId,
    confirmations: claim.confirmations > 0 ? claim.confirmations : 1,
    ...
    value: {
      ...
      source: {
        ...claim.value?.source,
        url: mediaUrl || claim.value?.source?.url,
      },
    },
  };
}

function absoluteHyperbeamUrl(url: any) {
