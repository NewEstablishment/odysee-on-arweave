...
%% ran (an inbound `cookie' header is stripped from the top level and stored under
%% `priv/cookie'); an empty private map counts as absent. A plain key is matched
%% at the top level. This lets a node gate on a browser cookie over the wire while
%% a request with neither the plain key nor a non-empty private value still skips.
